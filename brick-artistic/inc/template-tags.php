...
function brick_entry_author() {
	...
	echo apply_filters( 'brick_entry_author', $html ); // wpcs: xss ok.
}
 ...
 * with the 'brick_entry_date' filter.
 */
function brick_entry_date() {
	...
	echo apply_filters( 'brick_entry_date', $html ); // wpcs: xss ok.
}
 ...
 * categories attached to it. Can be filtered with 'brick_entry_categories'
 * filter.
 */
function brick_entry_categories() {
	...
	// Filter and display.
	echo apply_filters( 'brick_entry_categories', $html ); // wpcs: xss ok.
}

/**
 * Displays page views.
 */
function brick_entry_pageviews() {
 ...
 */
function brick_entry_comments_link() {
	if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		echo '<span class="entry-meta-item comments-link">';
			comments_popup_link(
				__( 'Leave a comment', 'brick' ),
				__( 'One Comment', 'brick' ),
				__( '% Comments', 'brick' )
			);
		echo '</span>';
	}
 ...
 */
function brick_entry_footer_widgets() {
 ...
 */
function brick_entry_author_bio() {
		...
		<div class="author-description">
			<h4 class="author-heading">
				<?php
					...
					printf( esc_html__( 'Published by %s', 'brick' ), '<span class="author-name">' . get_the_author() . '</span>' );
				?>
			</h4>
			...
			<p class="author-url">
				<a href="<?php echo esc_url( get_the_author_meta( 'user_url' ) ); ?>" rel="author" target="_blank">
					<?php esc_html_e( 'Visit website', 'brick' ); ?>
				</a>
			</p>
 ...
 */
function brick_entry_meta_index() {
	// Only display meta on posts.
	if ( 'post' == get_post_type() ) {

		echo '<div class="entry-meta entry-meta-index">';

		brick_entry_categories();

		brick_entry_date();

		brick_entry_pageviews();
 ...
 * Displays the post meta.
 */
function brick_entry_meta_before_title() {
	// Only display meta on posts.
	if ( 'post' == get_post_type() ) {
 ...
 */
function brick_entry_meta_before_content() {
	// Only display meta on posts.
	if ( 'post' == get_post_type() ) {
 ...
 */
function brick_entry_meta_after_content() {

	echo '<footer class="entry-footer">';

	brick_entry_tags();

	brick_entry_author_bio();
...
function brick_post_thumbnail() {
	...
	$html = sprintf(
		...
		get_the_post_thumbnail( null, 'brick-single-thumbnail' )
	);

	echo apply_filters( 'brick_post_thumbnail', $html ); // wpcs: xss ok.
}

/**
 * Displays post thumbnail.
 */
function brick_post_thumbnail_archive() {
	if ( post_password_required() || is_attachment() || ! has_post_thumbnail() ) {
		return;
	}
	...
	$html = sprintf(
		...
		get_the_post_thumbnail( null, 'brick-blog-thumbnail' )
	);

	echo apply_filters( 'brick_post_thumbnail_archive', $html ); // wpcs: xss ok.
}
 ...
 * page. Can be filtered with 'brick_post_navigation' filter.
 */
function brick_post_navigation() {
	// Check if whe have posts to navigate to.
	$prev = get_previous_post_link( '<div class="nav-pre"><span>' . esc_html__( 'Older Post', 'brick' ) . ' </span>%link</div>' );
	$next = get_next_post_link( '<div class="nav-nxt"><span>' . esc_html__( 'Newer Post', 'brick' ) . ' </span>%link</div>' );
	...
	echo apply_filters( 'brick_post_navigation', $html ); // wpcs: xss ok.
}
 ...
 */
function brick_posts_pagination( $class = '' ) {
	$class = $class ? $class . ' pagination' : 'pagination';

	$args = array(
		'prev_text' => brick_get_svg( array( 'icon' => 'arrow-left' ) ) . __( 'Prev', 'brick' ),
		'next_text' => __( 'Next', 'brick' ) . brick_get_svg( array( 'icon' => 'arrow-right' ) ),
		'echo' => false,
	);
 ...
 * @param string $class Class to apply to a pagination element.
 */
function brick_comments_pagination( $class = '' ) {
 ...
 */
function brick_first_post_link() {
	...
	printf(
		wp_kses(
			...
			__( 'Ready to publish your first post? <a href="%1$s">Get started here</a>.', 'brick' ),
			$args
		),
